...
import { SeoMetadata } from "../types.js";

// SEO Utilities for NEXT.JS (App Router) :  uses native Next.js Metadata API
export function buildNextMetadata(metadata: SeoMetadata): Metadata {
  // 1. Metadata Setup
  ...
  const type: (typeof allowedTypes)[number] = allowedTypes.includes(
    metadata.type as (typeof allowedTypes)[number]
  )
    ? (metadata.type as (typeof allowedTypes)[number])
    : "website";
